...
class BertForNer:
    ...
    def dev(self):
        ...
        all_attention_masks = []  # 新增：存储attention masks
        ...
        with torch.no_grad():
            for dev_batch_data in self.dev_loader:
                ...
                if self.args.use_crf == 'True':
                    # 获取实际的序列长度（非填充部分）
                    sequence_lengths = dev_batch_data['attention_masks'].sum(dim=1).tolist()

                    padded_predictions = []
                    for i, (pred, seq_len) in enumerate(zip(outputs[1], sequence_lengths)):
                        # 检查预测结果的类型
                        ...
                        # 调整序列长度
                        if len(pred) > seq_len:
                            padded_predictions.append(pred[:seq_len])
                        elif len(pred) < seq_len:
                            padded = pred + [0] * (seq_len - len(pred))
                            padded_predictions.append(padded)
                        else:
                            padded_predictions.append(pred)

                    all_predictions.extend(padded_predictions)
                else:
                    # 非CRF情况处理不变
                ...
                # 收集标签
                labels = dev_batch_data['labels'].cpu().numpy()

                # 使用attention masks过滤padding部分
                masks = dev_batch_data['attention_masks'].cpu().numpy()
    ...
    def test(self):
        ...
        with torch.no_grad():
            for test_batch_data in self.test_loader:  # 使用测试集加载器
                ...
                # 收集预测结果
                if self.args.use_crf == 'True':
                    sequence_lengths = test_batch_data['attention_masks'].sum(dim=1).tolist()
                    padded_predictions = []
                    for i, (pred, seq_len) in enumerate(zip(outputs[1], sequence_lengths)):
                        if isinstance(pred, torch.Tensor):
                            pred = pred.tolist()
                ...
                # 收集标签
                labels = test_batch_data['labels'].cpu().numpy()
                masks = test_batch_data['attention_masks'].cpu().numpy()
                for i in range(labels.shape[0]):
                    valid_indices = np.where(masks[i] == 1)[0]
                    valid_labels = labels[i][valid_indices]
                    all_labels.append(valid_labels)
        ...
        # 计算测试集性能
        precision, recall, f1 = self.compute_metrics(all_predictions, all_labels)
        return tot_test_loss / len(self.test_loader), precision, recall, f1
...
if __name__ == '__main__':
    # 设置数据集名称（应与预处理设置一致）
    data_name = "my_data_zh"  # 您的自定义数据集名称
    ...
    test_loss, precision, recall, f1 = bert_ner.test()
    logger.info(f"[Test] loss:{test_loss:.4f} precision={precision:.4f} recall={recall:.4f} f1_score={f1:.4f}")


"""# 预测示例
    example_text = "角闪石，辉石不均分布，石榴子石为铁铝质的钙铝榴石，其晶体成分及化学式见表。"  # 替换为您的领域相关文本
    entities = bert_ner.predict(example_text)
