...
class DataSet():
    ...
    def __getitem__(self, item):
        ...
        tags = self.tags[item]
        if item == 142:  # Debug sentence #142
            logger.info(f"Sentence #142: {sentence}")
            logger.info(f"Tags #142: {tags}")
        try:
            # Encode tags
            tags = self.tag_encoder.transform(tags)
        except ValueError as e:
            logger.error(f"Error encoding tags for sentence #{item}: {tags}")
            logger.error(f"Sentence: {sentence}")
            logger.error(f"Tag encoder classes: {self.tag_encoder.classes_}")
            raise e
...
def get_semeval_data(split: str = 'train',
    ...
    for line_num, line in enumerate(lines):
        line = line.strip()
        if line == '':
            if sentence and entity:  # Only append non-empty sentences
                if len(sentence) == len(entity):
                    sentences.append(sentence)
                    entities.append(entity)
                else:
                    logger.warning(f"Skipping sentence at line {line_num} due to length mismatch: {sentence}, {entity}")
            sentence = []
            entity = []
            continue

        line_parts = line.split()
        if len(line_parts) < max(word_idx, entity_idx) + 1:
            logger.warning(f"Skipping malformed line {line_num} in {file_path}: {line}")
            continue

        # Extract word and entity tag
        word = line_parts[word_idx]
        tag = line_parts[entity_idx]
        sentence.append(word)
        entity.append(tag)

    # Append the last sentence if it exists
    if sentence and entity and len(sentence) == len(entity):
        sentences.append(sentence)
        entities.append(entity)
    elif sentence or entity:
        logger.warning(f"Discarding incomplete final sentence: {sentence}, {entity}")
