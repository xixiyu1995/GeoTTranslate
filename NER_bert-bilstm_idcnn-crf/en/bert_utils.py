...
def get_ent_tags(all_tags):
    ...
    ent_tags = list(ent_tags)
    logger.info(f"Extracted tags: {ent_tags}")
    return ent_tags
...
def compute_f1(pred_tags, golden_tags, from_test=False):
    assert len(pred_tags) == len(
        golden_tags), f"Number of predicted sequences ({len(pred_tags)}) does not match number of ground truth sequences ({len(golden_tags)})"
    for i, (pred, golden) in enumerate(zip(pred_tags, golden_tags)):
        if len(pred) != len(golden):
            logger.error(
                f"Length mismatch at sequence {i}: predicted length {len(pred)}, ground truth length {len(golden)}")
            logger.error(f"Predicted tags: {pred}")
            logger.error(f"Ground truth tags: {golden}")
